...
A local Node.js/Express web app that replaces trackden.org as a personal Bandcamp library manager. Lets you build playlists, play tracks, manage a wishlist, and push/pull the Bandcamp cart — all from localhost:3000.
...
Handles three messages via `externally_connectable`:
...
## Smart playlists
`type: 'smart'` playlists have a `criteria` object instead of a `trackIds` array. `smartPlaylistTrackIds(criteria)` in `app.js` computes members dynamically from `Object.values(state.tracks)`. Criteria fields: `genre` (string), `purchased` (`'owned'|'unowned'`), `price` (`'free'|'paid'`), `addedWithin` (number of days).

Built-in smart playlists (Recently Added, Unowned, Free) are created by `ensureBuiltInSmartPlaylists()` on boot if not present. Smart playlists are never added to `state.sidebarOrder` — they render in a separate `#smart-playlist-list` section in the sidebar.
